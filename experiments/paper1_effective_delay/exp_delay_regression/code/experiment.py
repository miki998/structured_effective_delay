...
class Experiments:
    ...
    def run_experiment2(self):
        ...
        fig1, axes = plt.subplots(nrows=1, ncols=3, figsize=(10, 4))
        axes[0].imshow(x_ground_mat, cmap='gray')
        axes[0].set_title("Effective $x=\mathbf{1}$\n (if bundle)")
        add_cbar(fig1, axes[0])
        # utils.annotate_heatmap(fig1, axes[0], x_ground_mat, adapt_color=0.6)
        axes[1].imshow(x_pred_mat, cmap='gray')#, vmax=y_pred_mat.max())
        axes[1].set_title(f"Estimated Effective \n loss={np.round(loss,4)}")
        add_cbar(fig1, axes[1])
        axes[2].scatter(x_ground, x_opt, s=20, alpha=.5, edgecolors="none", color='blue')
        ...
        add_cbar(fig2, ax[1])
        ax[2].scatter(y_ground.numpy(), y_est.numpy(), s=20, alpha=.5, edgecolors="none", color='blue')
